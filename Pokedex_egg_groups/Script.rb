def draw_egg_group_in_pokedex(species_data, overlay)
	width = height = 32
	x = 208
	y = 200
	space = 4
	eggbitmap = AnimatedBitmap.new(_INTL("Graphics/Pictures/Pokedex/icon_egg_groups"))
	position = [
		:Amorphous,:Monster,:Flying,:Field,:Bug,:Dragon,:Mineral,:Water3,
		:Humanlike,:Water2,:Grass,:Unknown,:Fairy,:Water1,:Undiscovered,:Ditto
	]
	species_data.egg_groups.each_with_index do |group, i|
		if group != :Undiscovered
			egg_number = position.index(group)
			egg_rect = Rect.new(0, egg_number * height, width, height)
			overlay.blt(x + ((width + space) * i), y, eggbitmap.bitmap, egg_rect)
		end
	end
end
class PokemonPokedexInfo_Scene
	def drawPageInfo
		...
		if $player.owned?(@species)
			...
			# Draw the egg group icon(s) TK
			draw_egg_group_in_pokedex(species_data, overlay)
		else
			# Write the category
