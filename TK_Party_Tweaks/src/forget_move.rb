MenuHandlers.add(:party_menu, :forget_move, {
  ...
  "condition" => proc { |screen, party, party_idx| 
		next TKPartyTweaks::CAN_FORGET_MOVE && 
		!party[party_idx].egg? && 
		!party[party_idx].shadowPokemon? && 
		party[party_idx].numMoves > 1 &&
		(
			$player.party[party_idx].hasMove?(:AMNESIA) || 
			(
				$player.get_pokemon_with_move(:HYPNOSIS) &&
				!$player.party[party_idx].hasMove?(:HYPNOSIS)
			)
		)
	},
